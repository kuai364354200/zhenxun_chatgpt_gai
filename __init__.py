...
__plugin_settings__ = {"level": 1, "admin_level": 2, "default_status": True, "limit_superuser": False, "cmd": __plugin_cmd__, }
__plugin_cd_limit__ = {"cd": 600, "limit_type": "group", "rst": "世界树cd600s，如有需要请找550A他爹充钱"}
...
async def _(event: MessageEvent, arg: Message = CommandArg()):
    ...
    if len(response)>=response_max_len:
        ...
        #循环切片并分发
        for i in range(int(len(response)/response_max_len)):
            a_response = temp_response[0:response_max_len]
            temp_response = temp_response[response_max_len:]
            await ai.send(a_response, at_sender=True)
        #把最后一局发掉，结束
        await ai.finish(a_response, at_sender=True)
    else:
        await ai.finish(response, at_sender=True)
